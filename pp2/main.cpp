...
void heap_delete_tests(max_heap &hp) {
	...
	if (hp.size() > 1) {
		...
		std::cout << "top of heap is currently " <<  hp.top() << " and swapping with left child, freq=105" <<  std::endl;
		temp = hp.delete_max();
		std::cout << "Item returned from heap delete: " << temp << std::endl;
		temp = hp.top();
		std::cout << "Top of heap is now: " << temp << std::endl;
	...
	std::cout << "Top of heap is " << hp.top() << " and left child is 30, right child is 50" << std::endl;
	temp = hp.delete_max();
	std::cout << "Item returned from heap delete: " << temp << std::endl;
	temp = hp.top();
	std::cout << "Top of heap is now: " << temp << std::endl;
